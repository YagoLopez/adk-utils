import { GenAIAgentService } from './genai-agent-service';
import { UIMessage } from 'ai';
import { LlmAgent, BaseLlm, BaseTool } from '@google/adk';
...
describe('GenAIAgentService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new GenAIAgentService(mockAgent);
  });

  describe('validateMessages', () => {
    it('should return true for valid messages', () => {
      const messages: UIMessage[] = [{ role: 'user', id: '1', parts: [{ type: 'text', text: 'Hello' }] }];
  ...
  describe('formatToolsForGenAI', () => {
    ...
    it('should throw error if tool has no declaration', () => {
      ...
      const invalidAgent = { ...mockAgent, tools: [invalidTool] } as unknown as LlmAgent;
      const invalidService = new GenAIAgentService(invalidAgent);
  ...
  describe('createErrorResponse', () => {
    it('should create an error response', async () => {
      const response = GenAIAgentService.createErrorResponse('Error message', 500);
    ...
    it('should create an error response with details', async () => {
      const response = GenAIAgentService.createErrorResponse('Error message', 500, 'Details');
      expect(response.status).toBe(500);
      const body = await response.json();
  ...
  describe('Complex Flows & Private Methods', () => {
    ...
    describe('processTextDelta', () => {
      it('should handle non-overlapping text parts', () => {
        const processTextDelta = (service as any).processTextDelta.bind(service);
